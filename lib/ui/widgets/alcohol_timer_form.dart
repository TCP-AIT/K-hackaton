...
//   final List<String> _times = ['2시간 내','3시간 내','4시간 내','5시간 내','6시간 내','7시간 내','8시간 내','9시간 내','10시간 내','11시간 내','12시간 내'];
...
//     double drinkAmount = double.tryParse(_drinkAmountController.text) ?? 0;
//     double alcohol = double.tryParse(_alcoholController.text) ?? 0;
//     double timeNum = double.parse(_time.replaceAll('시간 내', ''));
//     double sexWeight = (_sex == '남성') ? 0.86 : 0.64;
//
//     double bac = ((drinkAmount*alcohol*0.01*0.7894*0.7)/(weight*sexWeight*10)-(0.015*(timeNum-1.5)));
//
//     if (bac<=0){
//       bac = 0;
//     }
...
class AlcoholTimerFormWidget extends StatefulWidget {
  bool isBtnActivated = false;

  AlcoholTimerFormWidget({super.key, required isBtnActivated});

  @override
  State<AlcoholTimerFormWidget> createState() => AlcoholTimerFormState();
}

class AlcoholTimerFormState extends State<AlcoholTimerFormWidget> {
  String _sex = '남성';
  double _sexWeight = 0;
  double _weight = 0;
  String _drink = '소주';
